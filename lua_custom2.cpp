...
#define sheet(num) if(L == clws) { lognum(num); }; 	dicks();
FILE*logger = 0;
void lognum(int num)
{
...
int __cdecl  hooked_lua_type(lua_State *L, int edx)
{
	sheet(30);
...
void* __cdecl  hooked_lua_rawgeti(lua_State *L, int idx, int r)
{
	...
	if (coolbridge_enabled(L))
	{
		return origlua_rawgeti(ws, idx, r);
	}
